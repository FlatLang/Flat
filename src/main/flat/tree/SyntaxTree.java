...
/**
 * Class that deconstructs a source code file and builds a Tree out of the statements.
 *
...
public class SyntaxTree {
    ...
    public static final Class<?> SCOPE_CHILD_DECODE[] = new Class<?>[] {
        ...
        Fallthrough.class, Continue.class, LambdaExpression.class, CodeBlock.class,
    ...
    public static final Class<?> FIRST_PASS_CLASSES[] = new Class<?>[] {
        DataClassDeclaration.class, Annotation.class, Import.class, ExtensionDeclaration.class,
        ClassDeclaration.class, InterfaceDeclaration.class, Trait.class,
        Package.class, ExternalCodeBlock.class
    };

    public static final Class<?> SECOND_PASS_CLASSES[] = new Class<?>[] {
        ArrayBracketOverload.class, Annotation.class, StaticBlock.class, DataClassDeclaration.class,
        ClassDeclaration.class, InterfaceDeclaration.class, Trait.class,
        AbstractMethodDeclaration.class,
        ExternalMethodDeclaration.class, ClosureVariable.class, ExtensionMethodDeclaration.class,
        ExtensionFieldDeclaration.class, Destructor.class, Constructor.class,
        BodyMethodDeclaration.class,
        ExternalType.class, FieldDeclaration.class, ExternalCodeBlock.class
    };

    /**
     * Create a SyntaxTree instance from the source code in the specified File.
     *
     * @param files The files containing the Foxy source code.
     * @param controller The controller of the compiling program.
     * @throws IOException Thrown if there was an error finding the files or reading from it.
     */
    ...
    /**
     * Generate a SyntaxTree instance given the name of the file and the source within it.
     *
     * @param files The files containing the Foxy source code.
    ...
    /**
     * Generate the SyntaxTree given the name of the file and the source within it.
     */
    public void generate() {
        this.useThreads = !controller.isFlagEnabled(Flat.SINGLE_THREAD);
        phaseInstanceDeclarationsWeight = useThreads && PHASE_INSTANCE_DECLARATIONS_PARALLEL
        ...
        try {
            ...
            root.forEachVisibleListChild(file -> Arrays.stream(file.getClassDeclarations())
                .forEach(ClassDeclaration::removeNonConcreteProperties));

            controller.processStep(
                Math.max(controller.getEstimatedStepsToProcess(), controller.getStepsToProcess())
                    - controller.getProcessedSteps());
            controller.log("", true);
    ...
    /**
     * Initialize the TreeGenerator objects that will be used to generate the trees concurrently (if
     * threads are used).
     *
     * @param files The files to generate trees for.
     * @param sources The sources within the files to generate the trees for.
     */
     ...
     * </ul>
     *
     * @return The phase (1, 2, or 3) that the SyntaxTree is currently generating in.
     */
    public int getPhase() {
        return phase;
    }
    ...
    /**
     * Generate the SyntaxTree data for the specified phase. A phase specified what section of code
     * that the compiler is decoding.<br>
     * For instance:
     * <ul>
     * <li><b>Phase 1</b>: Decode imports and class headers</li>
     * <li><b>Phase 2</b>: Decode fields, external types, and method declarations</li>
    ...
    private void phase(int phase) throws InterruptedException {
        ...
        if (phase == PHASE_CLASS_DECLARATION) {
            ...
            if (Flat.PRIMITIVE_OVERLOADS) {
                ...
                root.forEachVisibleListChild(
                    file -> Arrays.stream(file.getClassDeclarations()).forEach(c -> {
                        c.convertProperties();
                        controller.processStep();
                    }));
            }
        ...
        if (phase == PHASE_INSTANCE_DECLARATIONS) {
            ...
            controller.setEstimatedStepsToProcess(
                controller.getStepsToProcess() +
                    (int) root.getFilesStream()
                        ...
                        .count()
                        * (useThreads ? 20 / Runtime.getRuntime().availableProcessors() : 15)
                    +
                    (int) root.getFilesStream()
                        .flatMap(f -> Arrays.stream(f.getClassDeclarations()))
                        .count() * 20
            ...
            runActions(
                root.getChildStream().map(f -> (FileDeclaration) f)
                    ...
                    .map(file -> () -> Arrays.stream(file.getClassDeclarations()).forEach((c) -> {
                        controller.log("Creating static class instance declaration for class "
                            + c.getClassLocation() + "...");
                        classes.add(c);

                        c.classInstanceDeclaration =
                            new ClassInstanceDeclaration(c, Location.INVALID);
                        c.classInstanceDeclaration.setStatic(true);
                        c.classInstanceDeclaration.setLazy(true);

                        String extendedClassName =
                            c.getExtendedClass() != null ? c.getExtendedClassName() + ".class"
                                : "null";
                        ...
                        String interfacesArg =
                            interfaceValues.length() > 0 ? "[" + interfaceValues + "]" : "Array()";
                        ...
                        String fieldsArg =
                            fieldValues.length() > 0 ? "[" + fieldValues + "]" : "Array()";
                        ...
                        String functionsArg =
                            functionValues.length() > 0 ? "[" + functionValues + "]" : "Array()";
                        ...
                        if (controller.target.equalsIgnoreCase("js")
                            || controller.target.equalsIgnoreCase("es6")) {
                            extra = ", this";
                        }

                        c.classInstanceDeclaration.setShorthandAccessor(
                            "Class<" + c.getName() + ">(\"" + c.getClassLocation() + "\", "
                                + isInterfaceValue + ", " + extendedClassName + ", " + interfacesArg
                                + ", " + fieldsArg + ", " + functionsArg + extra + ")");
            ...
            classes.stream()
                ...
                .forEach(c -> {
                    metaClass.getFileDeclaration().addImport(c.getClassLocation(), false,
                        "Class" + index[0]++, true);
                });
            ...
            String classesValue = "["
                + classes.stream().filter(c -> c.encapsulatingClass == null)
                    .map(c -> "Class" + index[0]++ + ".class").collect(Collectors.joining(", "))
                + "]";
                ...
                root.getFilesStream()
                    ...
                    .map(file -> () -> Arrays.stream(file.getClassDeclarations()).forEach((c) -> {
                        if (!c.classInstanceDeclaration.containsAccessorMethod()) {
                            controller
                                .log("Compiling class instance declaration arrow binding for class "
                                    + c.getClassLocation() + "...");
            ...
            root.forEachVisibleListChild(file -> Arrays.stream(file.getClassDeclarations())
                ...
                .forEach(c -> {
                    controller
                        .log("Compiling arrow bindings for class " + c.getClassLocation() + "...");
    ...
    /**
     * Search for the main method, if one exists, in the compiling program. For more details on what
     * the main method looks like, see {@link SyntaxUtils#isMainMethod(BodyMethodDeclaration)}.
     *
    ...
    public BodyMethodDeclaration getMainMethod(String mainClass) {
        ...
        for (BodyMethodDeclaration method : mainMethods) {
            if (main != null) {
                if (!printedError) {
                    SyntaxMessage.error(
                        "Multiple main methods found. Please specify which one you want to run by passing a -main argument to the compiler",
                        main, false);
                }
                SyntaxMessage.error(
                    "Multiple main methods found. Please specify which one you want to run by passing a -main argument to the compiler",
    ...
    /**
     * Decode the specific statement into its correct Node value. If the statement does not
     * translate into a Node, a syntax error has occurred.
     *
     * @param parent The Parent Node of the current statement to be decoded.
     * @param statement The statement to be decoded into a Node.
     * @param location The Location in the source text where the statement is located at.
     * @param types The types of Nodes to try to decode, in the given order.
     ...
     */
    public static Node decodeStatement(Node parent, String statement, Location location,
        boolean require, Class<?> types[]) {
        return decodeStatement(parent, statement, location, require, types, true);
    }

    public static Node decodeStatement(Node parent, String statement, Location location,
        boolean require, Class<?> types[], boolean checkType) {
        Node node = null;
        ...
        for (Class<?> type : types) {
            ...
            if (type == LocalDeclaration.class)
                node = LocalDeclaration.decodeStatement(parent, statement, location, require, true,
                    checkType);
                ...
                node =
                    AbstractMethodDeclaration.decodeStatement(parent, statement, location, require);
                ...
                node =
                    ExternalMethodDeclaration.decodeStatement(parent, statement, location, require);
                ...
                node =
                    ExtensionFieldDeclaration.decodeStatement(parent, statement, location, require);
            else if (node == null && type == ExtensionMethodDeclaration.class)
                node = ExtensionMethodDeclaration.decodeStatement(parent, statement, location,
                    require);
     ...
     * @return The Node constructed from the statement, if any.
     */
    public static Node decodeStatementOfType(Node parent, String statement, Location location,
        boolean require, Class<?> type) {
        Node node = null;

        if (type.isAssignableFrom(Loop.class)
            && (node = Loop.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Instantiation.class)
            && (node = Instantiation.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(IfStatement.class)
            && (node = IfStatement.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(AbstractMethodDeclaration.class)
            && (node = AbstractMethodDeclaration.decodeStatement(parent, statement, location,
                require)) != null)
            ;
        else if (type.isAssignableFrom(Cast.class)
            && (node = Cast.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ArrayAccess.class)
            && (node = ArrayAccess.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ArrayBracketOverload.class) && (node =
            ArrayBracketOverload.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ArrayAccessorMethod.class) && (node =
            ArrayAccessorMethod.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ArrayMutatorMethod.class) && (node =
            ArrayMutatorMethod.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Assignment.class)
            && (node = Assignment.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Array.class)
            && (node = Array.decodeStatement(parent, statement, location, require)) != null)
            ;
        // else if (type.isAssignableFrom(Bool.class) && (node = Bool.decodeStatement(parent,
        // statement, location, require)) != null);
        else if (type.isAssignableFrom(Break.class)
            && (node = Break.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Case.class)
            && (node = Case.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ClosureVariable.class) && (node =
            ClosureVariable.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Continue.class)
            && (node = Continue.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(BinaryOperation.class) && (node =
            BinaryOperation.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ClassDeclaration.class) && (node =
            ClassDeclaration.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(CodeBlock.class)
            && (node = CodeBlock.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Constructor.class)
            && (node = Constructor.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Default.class)
            && (node = Default.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Destructor.class)
            && (node = Destructor.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ElseStatement.class)
            && (node = ElseStatement.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(DataClassDeclaration.class) && (node =
            DataClassDeclaration.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ExtensionDeclaration.class) && (node =
            ExtensionDeclaration.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ExternalCodeBlock.class) && (node =
            ExternalCodeBlock.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ExternalMethodDeclaration.class)
            && (node = ExternalMethodDeclaration.decodeStatement(parent, statement, location,
                require)) != null)
            ;
        else if (type.isAssignableFrom(ExternalType.class)
            && (node = ExternalType.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Fallthrough.class)
            && (node = Fallthrough.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Import.class)
            && (node = Import.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(InterfaceDeclaration.class) && (node =
            InterfaceDeclaration.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Trait.class)
            && (node = Trait.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(IntRange.class)
            && (node = IntRange.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(LambdaExpression.class) && (node =
            LambdaExpression.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Literal.class)
            && (node = Literal.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(LocalDeclaration.class) && (node =
            LocalDeclaration.decodeStatement(parent, statement, location, require)) != null)
            ;
        // else if (type.isAssignableFrom(Null.class) && (node = Null.decodeStatement(parent,
        // statement, location, require)) != null);
        else if (type.isAssignableFrom(MethodCall.class)
            && (node = MethodCall.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(BodyMethodDeclaration.class) && (node =
            BodyMethodDeclaration.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ExtensionFieldDeclaration.class)
            && (node = ExtensionFieldDeclaration.decodeStatement(parent, statement, location,
                require)) != null)
            ;
        else if (type.isAssignableFrom(ExtensionMethodDeclaration.class)
            && (node = ExtensionMethodDeclaration.decodeStatement(parent, statement, location,
                require)) != null)
            ;
        else if (type.isAssignableFrom(Priority.class)
            && (node = Priority.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Return.class)
            && (node = Return.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(UnaryOperation.class) && (node =
            UnaryOperation.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(FieldDeclaration.class) && (node =
            FieldDeclaration.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Catch.class)
            && (node = Catch.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(ExceptionHandler.class) && (node =
            ExceptionHandler.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Finally.class)
            && (node = Finally.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(StaticBlock.class)
            && (node = StaticBlock.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Super.class)
            && (node = Super.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Match.class)
            && (node = Match.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Throw.class)
            && (node = Throw.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Try.class)
            && (node = Try.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(TernaryOperation.class) && (node =
            TernaryOperation.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type.isAssignableFrom(Until.class)
            && (node = Until.decodeStatement(parent, statement, location, require)) != null)
            ;
     ...
     * @return The Node constructed from the statement, if any.
     */
    public static Node decodeVariable(Node parent, String statement, Location location,
        boolean require, boolean declaration) {
        Class<?> type = Identifier.class;
        Class<?> type2 = VariableDeclaration.class;

        Node node = null;

        if (type2.isAssignableFrom(AbstractMethodDeclaration.class) == declaration
            && type.isAssignableFrom(AbstractMethodDeclaration.class)
            && (node = AbstractMethodDeclaration.decodeStatement(parent, statement, location,
                require)) != null)
            ;
        else if (type2.isAssignableFrom(ArrayAccess.class) == declaration
            && type.isAssignableFrom(ArrayAccess.class)
            && (node = ArrayAccess.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(Array.class) == declaration
            && type.isAssignableFrom(Array.class)
            && (node = Array.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(ClassDeclaration.class) == declaration
            && type.isAssignableFrom(ClassDeclaration.class) && (node =
                ClassDeclaration.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(Closure.class) == declaration
            && type.isAssignableFrom(Closure.class)
            && (node = Closure.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(ClosureVariable.class) == declaration
            && type.isAssignableFrom(ClosureVariable.class) && (node =
                ClosureVariable.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(CodeBlock.class) == declaration
            && type.isAssignableFrom(CodeBlock.class)
            && (node = CodeBlock.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(Constructor.class) == declaration
            && type.isAssignableFrom(Constructor.class)
            && (node = Constructor.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(Destructor.class) == declaration
            && type.isAssignableFrom(Destructor.class)
            && (node = Destructor.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(ExternalMethodDeclaration.class) == declaration
            && type.isAssignableFrom(ExternalMethodDeclaration.class)
            && (node = ExternalMethodDeclaration.decodeStatement(parent, statement, location,
                require)) != null)
            ;
        else if (type2.isAssignableFrom(Instantiation.class) == declaration
            && type.isAssignableFrom(Instantiation.class)
            && (node = Instantiation.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(LambdaExpression.class) == declaration
            && type.isAssignableFrom(LambdaExpression.class) && (node =
                LambdaExpression.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(LocalDeclaration.class) == declaration
            && type.isAssignableFrom(LocalDeclaration.class) && (node =
                LocalDeclaration.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(MethodCall.class) == declaration
            && type.isAssignableFrom(MethodCall.class)
            && (node = MethodCall.decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(BodyMethodDeclaration.class) == declaration
            && type.isAssignableFrom(BodyMethodDeclaration.class) && (node = BodyMethodDeclaration
                .decodeStatement(parent, statement, location, require)) != null)
            ;
        else if (type2.isAssignableFrom(ExtensionMethodDeclaration.class) == declaration
            && type.isAssignableFrom(ExtensionMethodDeclaration.class)
            && (node = ExtensionMethodDeclaration.decodeStatement(parent, statement, location,
                require)) != null)
            ;
        else if (type2.isAssignableFrom(ExtensionFieldDeclaration.class) == declaration
            && type.isAssignableFrom(ExtensionFieldDeclaration.class)
            && (node = ExtensionFieldDeclaration.decodeStatement(parent, statement, location,
                require)) != null)
            ;
        else if (type2.isAssignableFrom(FieldDeclaration.class) == declaration
            && type.isAssignableFrom(FieldDeclaration.class) && (node =
                FieldDeclaration.decodeStatement(parent, statement, location, require)) != null)
            ;
    ...
    /**
     * Decode a String that was found within a scope. That is, a method or scopes within a method:
    ...
    private static Object getArrayAccessOrNode(Node parent, String statement, Location location,
        boolean require) {
    ...
    public static Node decodeScopeContents(Node parent, String statement, Location location,
        boolean require, boolean checkType) {
        ...
        if (accessString != null && node instanceof Value) {
            Node access = ArrayAccess.decodeStatement(((Value) node).getReturnedNode(),
                accessString, location, require);
            ...
            if (access == null) {
                ...
                ((Accessible) ((Value) node).getReturnedNode())
                    .setAccessedNode((MethodCall) access);
            }
        }
     ...
     * </pre>
     * 
     * </blockquote> The above expression is an identifier access because it uses the dot operator
     * to access another node.
     *
     ...
     * @param require Whether or not to throw an error if anything goes wrong.
     */
    public static Accessible decodeIdentifierAccess(Node parent, String statement,
     ...
     */
    public static Accessible decodeIdentifierAccess(Node parent, String statement,
        Location location, boolean require, boolean validateAccess) {
        return decodeIdentifierAccess(parent, statement, location, require, validateAccess, true);
    }

    public static String[][] getPrecedingModifiers(String statement, Node parent,
        Location location) {
    ...
    public static String[][] getPrecedingModifiers(String statement, Node parent, Location location,
        int startSkipCount, int endSkipCount) {
        ...
        final IIdentifier n = new IIdentifier(parent, location) {
            ...
            public boolean interactWord(String word, Bounds bounds, String leftDelimiter,
                ...
                if (leftDelimiter.length() == 0 && extra.getWordNumber() < startSkipCount) {
                    ...
                    return false;
                } else if (leftDelimiter.length() == 0 && !rightDelimiter.equals(".")
                    && !rightDelimiter.equals(":")) {
                    if (isModifier(word)) {
                        modifiers.add(word);
                        return true;
                    } else {
        ...
        Node.ExtraData modifierData =
            n.iterateWords(statement, Patterns.IDENTIFIER_BOUNDARIES, null, false);
    ...
    public static Accessible decodeIdentifierAccess(Node parent, String statement,
        Location location, boolean require, boolean validateAccess, boolean requireDot) {
        ...
        while (current != null) {
            ...
            if (root == null) {
                ...
                if ((cast =
                    checkAutoCasts((Node) node, (Value) parent, root, "FunctionMap")) != root ||
                    (cast =
                        checkAutoCasts((Node) node, (Value) parent, root, "PropertyMap")) != root) {
                    root = cast;
                    parent = (Node) cast;
                } else {
                    parent = node.getReturnedNode();
    ...
    private static Accessible checkAutoCasts(Node node, Value accessing, Accessible root,
        String type) {
        if ("flat/meta/Class".equals(accessing.getTypeClassLocation())
            ...
            if (v.getName() != null && v.getName().equals(lower)) {
                if (((Accessible) accessing).getCast() == null) {
                    ...
                    if (!accessing.getFileDeclaration().containsImport(funMap, false)) {
                        accessing.getFileDeclaration()
                            .addImport(arg.getTypeClassLocation() + "." + funMap);
                    }
    ...
    /**
     * Decode the value of the given statement. Can be nodes such as Variables, ExternalTypes,
     * Fields, etc. May also be just a plain old Value describing the return type of the statement.
     * For example: a static class's method call. "<u><code>Math.sin(number)</code></u>" in this
     * instance, Math will be the Identifier returned. In most other instances a Variable variation
     * will be returned.
     *
     * @param parent The parent node of the current statement to decode.
     * @param statement The statement to decode.
    ...
    public static Accessible decodeAccessible(Node parent, String statement, Location location,
        ...
        Accessible node =
            (Accessible) decodeStatementOfType(parent, statement, location, require, Super.class);

        if (node == null) {
            node = decodeVariable(parent, statement, location, validateAccess);

            if (node == null) {
                node = (Accessible) Literal.decodeStatement(parent, statement, location, require,
                    true);

                if (node == null) {
                    node = (Accessible) decodeStatementOfType(parent, statement, location, require,
                        Priority.class);

                    if (node == null) {
                        node =
                            (Accessible) Array.decodeStatement(parent, statement, location, false);

                        if (node == null) {
                            node = StaticClassReference.decodeStatement(parent, statement, location,
                                false);

                            if (node == null) {
                                node = ReifiedParameterAccess.decodeStatement(parent, statement,
                                    location, false);
                            }
                        }
                    }
    ...
    public static Identifier decodeIdentifier(Node parent, String statement, Location location,
        ...
        Identifier node =
            SyntaxTree.getUsableExistingNode(parent, statement, location, validateAccess);
        ...
        if (node == null) {
            if (parent instanceof ExternalType) {
                ...
                node = id;
            } else if (parent.getFileDeclaration(true).containsImport(statement, false)
                || parent.getFileDeclaration(true).containsClass(statement)) {
                ClassDeclaration clazz =
                    SyntaxUtils.getImportedClass(parent.getFileDeclaration(true), statement);

                if (clazz != null) {
                    StaticClassReference reference =
                        StaticClassReference.decodeStatement(parent, statement, location, true);
    ...
    /**
     * Try to find an existing node from the given statement. This method searches through fields
     * and local variables.
     *
    ...
    /**
     * Try to find an existing Node's declaration from the given statement. This method searches
     * through fields and local variables.
     *
     * @param parent The parent Node to use as our context.
     * @param statement The statement to check for an existing declaration from.
    ...
    public static VariableDeclaration findDeclaration(Node parent, String statement,
        ...
        if (statement.equals(LambdaExpression.UNNAMED_PARAMETER)) {
            if (parent.getParentMethod(true) instanceof LambdaMethodDeclaration) {
                LambdaMethodDeclaration method =
                    (LambdaMethodDeclaration) parent.getParentMethod(true);
            ...
            if (parent.getParentMethod(true) instanceof LambdaMethodDeclaration) {
                ...
                Parameter param = method.getParameterList()
                    .getVisibleChild(Integer.parseInt(statement.substring(1)) - 1);
        ...
        if (statement.equals("class")) {
            if (parent instanceof StaticClassReference)// !(parent instanceof Accessible) ||
                                                       // !((Accessible)parent).canAccess())
            {
                        ...
                        if (node == null && clazz instanceof ExtensionDeclaration) {
                            ...
                            if (method != null && node == null) {
                                ReferenceParameter referenceParam =
                                    method.getParameterList().getReferenceParameter();
                                ClassDeclaration typeClazz = referenceParam.getTypeClass();

                                if (typeClazz != null
                                    && typeClazz instanceof ExtensionDeclaration == false) {
     ...
     * @param validateAccess Whether or not to check if method call can be accessed legally.
     * @return The found Field. If the Field was not found, null is returned.
     */
    private static InstanceDeclaration checkForVariableAccess(Node parent, String statement,
        boolean validateAccess) {
    ...
    private static InstanceDeclaration validateFieldAccess(Node parent,
        ClassDeclaration accessingClass, InstanceDeclaration field) {
        if (field != null) {
     ...
     * @return The found local variable declaration. If the local variable was not found, null is
     *         returned.
     */
     ...
     * </pre>
     * 
     * </blockquote> The above expression would be returned in a Value Node.
     *
     * @param parent The parent of the statement to decode.
     ...
     * @param require Whether or not to throw an error if anything goes wrong.
     */
    public static Value decodeValue(Node parent, String statement, Location location,
        boolean require) {
    ...
    public static Value decodeValue(Node parent, String statement, Location location,
        ...
        /*
         * if (node instanceof Variable) { if (((Variable)node).getDeclaration() instanceof
         * ClosureDeclaration) { return new Closure((Variable)node); } }
         */
        ...
        if (node instanceof Value) {
            ...
            SyntaxMessage.queryError("Invalid operation '" + statement + "'", parent, location,
                require);
        }
    ...
    /**
     * Output a generic error stating that the given syntax could not be decoded.
     *
    ...
    /**
     * Get the filenames of all the files, in order of the way they are in the tree structure
     *
     * @return The filenames of all the files, in order of the way they are in the tree structure.
     */
    ...
    /**
     * Get the root Program of the tree. The root of the Syntax tree will be a Program.
     *
    ...
    /**
     * Test the SyntaxTree class type to make sure everything is working properly.
     *
