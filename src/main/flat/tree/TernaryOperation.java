...
public class TernaryOperation extends IValue implements Accessible {
    ...
    /**
     * Decode the given statement into a {@link TernaryOperation} instance, if possible. If it is
     ...
     * @param statement The statement to try to decode into a {@link TernaryOperation} instance.
     * @param location The location of the statement in the source code.
     * @param require Whether or not to throw an error if anything goes wrong.
     * @return The generated node, if it was possible to translated it into a
     *         {@link TernaryOperation}.
     */
    public static TernaryOperation decodeStatement(Node parent, String statement, Location location,
        boolean require) {
        ...
        if (questionMarkIndex > 0) {
            ...
            if (colonIndex > questionMarkIndex) {
                ...
                Value condition =
                    SyntaxTree.decodeValue(parent, conditionString, location, require);

                if (condition == null) {
                    SyntaxMessage.queryError(
                        "Unable to decode ternary operation condition '" + conditionString + "'", n,
                        require);
                ...
                String trueValueString =
                    statement.substring(questionMarkIndex + 1, colonIndex).trim();
                ...
                } else // elvis operator
                {
                    ...
                    trueValue =
                        local.getDeclaration().generateUsableVariable(n, condition.getLocationIn());
                ...
                Value falseValue =
                    SyntaxTree.decodeValue(parent, falseValueString, location, require);

                if (trueValue == null || falseValue == null) {
                    SyntaxMessage.queryError(
                        "Unable to decode ternary operation false value '" + conditionString + "'",
                        n, require);
                ...
                Value v = Literal.isNullLiteral(trueReturnedValue) ? falseReturnedValue
                    : trueReturnedValue;
                ...
                if (Literal.isNullLiteral(trueValue)) {
                    ...
                    if (trueReturnedValue.getGenericTypeParameter() == falseReturnedValue
                        .getGenericTypeParameter()
                        && trueReturnedValue.getGenericTypeParameter() != null) {

                    } else if (trueReturnedValue.getTypeClass() == null
                        || falseReturnedValue.getTypeClass() == null) {
                        return null;
                    }

                    commonClass =
                        SyntaxUtils.getTypeInCommon(trueReturnedValue, falseReturnedValue);
                }

                n.setType(v); // Set the array type data and stuff like that
                n.setTypeValue(commonClass.getType()); // Set the actual class type to the common
                                                       // class
    ...
    public static Variable getLocalVariableFromNullCheck(BinaryOperation nullCheck) {
        return (Variable) ((Assignment) ((Priority) nullCheck.getLeftOperand()).getContents())
            .getAssigneeNode();
    }
    ...
    public StringBuilder generateFlatInput(StringBuilder builder, boolean outputChildren) {
        ...
        return getCondition().generateFlatInput(builder).append(" ? ")
            .append(getTrueValue().generateFlatInput()).append(" : ")
            .append(getFalseValue().generateFlatInput());
    }
    ...
    /**
     * Fill the given {@link TernaryOperation} with the data that is in the specified node.
     *
    ...
    /**
     * Test the {@link TernaryOperation} class type to make sure everything is working properly.
     *
