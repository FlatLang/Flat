...
public class ChainedMethodCall extends MethodCall
{
	...
	public Identifier getChainReference()
	{
		...
		{
			FunctionType type = (FunctionType)base.getFlatTypeValue(base).getTypeObject();
	...
	@Override
	public StringBuilder generateFlatInputUntil(StringBuilder builder, Accessible stopAt)
	{
		...
		return super.generateFlatInputUntil(builder, stopAt);
	}
...
//					VariableDeclaration declaration = call.getCallableDeclaration().toDeclaration();
					...
					Value funcType = call.getFlatTypeValue(call);
	...
	{
		super.generateFlatInput(builder, false);
	...
	public StringBuilder generateChain(StringBuilder builder)
	{
		...
		{
			builder.append("(");
			chained.getArgumentList().generateFlatInput(builder);
			builder.append(")");
	...
	public ValidationResult validate(int phase)
	{
		...
		if (phase == SyntaxTree.PHASE_PRE_GENERATION)
		{
			if (requiresLocal())//chained != null)
			{
				...
				if (chained == null)
				{
					variable.declaration.setType(declaration.generateFlatInput().toString());
				}
				else
				{
