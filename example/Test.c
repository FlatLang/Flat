...
void fathom_Test_main(ExceptionData* exceptionData, String** fathom_args_3)
{
		...
		Thread* fathom_thread_3;
		Thread* fathom_thread2_3;
		...
		fathom_thread_3 = fathom_Thread_Thread(exceptionData, 300, fathom_String_String(exceptionData, "Turkey"));
		fathom_thread2_3 = fathom_Thread_Thread(exceptionData, 1000, fathom_String_String(exceptionData, "Poop"));
		fathom_Thread_start(fathom_thread_3, exceptionData);
		fathom_IO_println(exceptionData, fathom_String_String(exceptionData, "Waiting for thread to end..."));
		fathom_Thread_join(fathom_thread_3, exceptionData);
		fathom_IO_println(exceptionData, fathom_String_String(exceptionData, "Thread has ended"));
		fathom_Thread_start(fathom_thread2_3, exceptionData);
		fathom_Thread_join(fathom_thread2_3, exceptionData);
...
{
		if (fathom_denominator_50 == 0)
		{
				THROW(2);
		}
		return fathom_numerator_50 / fathom_denominator_50;
}

int fathom_Test_getEvenNumber(Test* this, ExceptionData* exceptionData, int fathom_num_53)
{
		if (fathom_num_53 % 2 != 0)
		{
				THROW(3);
		}
		return fathom_num_53;
}

int fathom_Test_test(Test* this, ExceptionData* exceptionData)
{
		int fathom_newVar_56;
		
		fathom_newVar_56 = 43;
		return fathom_newVar_56;
}
