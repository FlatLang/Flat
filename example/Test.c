...
{
	List* fathom_list_85;
	ListNode* fathom_current_85;
	long_long fathom_start_85;
	int fathom_q_85;
	long_long fathom_end_85;
	
	fathom_list_85 = fathom_List_List(exceptionData);
	fathom_current_85 = fathom_List_getFirst(fathom_list_85, exceptionData);
	while (fathom_current_85 != 0)
	{
		fathom_ListNode_getData(fathom_current_85, exceptionData);
		fathom_current_85 = fathom_ListNode_getNext(fathom_current_85, exceptionData);
	}
	fathom_IO_println(exceptionData, fathom_String_String(exceptionData, "Starting benchmark..."));
	fathom_start_85 = fathom_Time_currentTimeMillis(exceptionData);
	for (fathom_q_85 = 99999999; fathom_q_85 >= 0; --fathom_q_85)
	{
		fathom_Math_sin(exceptionData, fathom_q_85);
	}
	fathom_end_85 = fathom_Time_currentTimeMillis(exceptionData);
	fathom_IO_printl(exceptionData, fathom_end_85 - fathom_start_85);
...
{
	if (fathom_denominator_96 == 0)
	{
		THROW(2);
	}
	return fathom_numerator_96 / fathom_denominator_96;
}

int fathom_Test_getEvenNumber(Test* this, ExceptionData* exceptionData, int fathom_num_108)
{
	if (fathom_num_108 % 2 != 0)
	{
		THROW(3);
	}
	return fathom_num_108;
}

int fathom_Test_test(Test* this, ExceptionData* exceptionData)
{
	int fathom_newVar_111;
	
	fathom_newVar_111 = 43;
	return fathom_newVar_111;
}
