...
public class StringUtils
{
	...
	public static Bounds findContentBoundsWithin(String source, String start, String end, int index, boolean includeEndings, char escapeChar)
	{
		...
		int s = findStringInBaseScope(source, start, index);
...
//			}
	...
	public static String[] splitCommas(String src, int searchGenerics, boolean allowTrailing)
	{
		...
		while ((index = findCharInBaseScope(src, ',', index + 1, searchGenerics)) >= 0)//(index = Regex.indexOf(src, index + 1, ',', new char[] { '(' }, new char[] { ')' }, new char[] { '"' }, new boolean[] { false }, new boolean[] { false }, new boolean[] { true })) > 0)
		{
	...
	public static String[] splitWhitespace(String src, int searchGenerics, boolean allowTrailing)
	{
		...
		while ((index = findCharInBaseScope(src, WHITESPACE, index + 1, searchGenerics)) >= 0)//(index = Regex.indexOf(src, index + 1, ',', new char[] { '(' }, new char[] { ')' }, new char[] { '"' }, new boolean[] { false }, new boolean[] { false }, new boolean[] { true })) > 0)
		{
			builder = new StringBuilder(src.substring(oldIndex, index));
