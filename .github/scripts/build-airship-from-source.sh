...
function visit(packageName) {
  if (packageName === 'Flat' || seen.has(packageName)) return;

  const packageFile = path.join(packagesDir, packageName, 'flat.json');
...
for (const packageName of [...seen].sort()) {
  ...
  const sourcePath = path.join(packagesDir, packageName, source.source);
