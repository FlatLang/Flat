...
public class UnaryOperation extends IValueValidator
{
	 ...
	 */
	public UnaryOperation(NodeValidator temporaryParent, Location locationIn)
	{
	...
	public ValueValidator getValue()
	{
		...
		{
			NodeValidator child = getChild(i);
			
			if (!(child instanceof Operator))
			{
				return (ValueValidator)child;
			}
		}
	 ...
	 */
	public static UnaryOperation decodeStatement(NodeValidator parent, String statement, Location location, boolean require)
	{
	...
	{
		ValueValidator value = SyntaxTree.decodeValue(this, contents, location, false);
		...
		else if (value instanceof BinaryOperationValidator)
		{
			return false;
		}
	...
	public ValidationResult validate(int phase)
	{
		...
		if (isOperatorOnLeft() && getOperator().getOperator().equals(Operator.BANG))
		{
			...
			{
				BinaryOperationValidator operation = getValue().replaceWithNullCheck();
				
				operation.getOperator().setOperator(Operator.EQUALS);
				
				replaceWith(operation);
				
				result.returnedNodeValidator = operation;
			}
		}
	 ...
	 */
	@Override
	public UnaryOperation clone(NodeValidator temporaryParent, Location locationIn, boolean cloneChildren, boolean cloneAnnotations)
	{
