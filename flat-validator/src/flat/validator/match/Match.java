...
public class Match extends ControlStatement
{
	 ...
	 */
	public Match(NodeValidator temporaryParent, Location locationIn)
	{
	...
	public boolean pendingScopeFragment(NodeValidator nodeValidator)
	{
		...
		NodeValidator ancestor = nodeValidator.getAncestorWithScope();
	 ...
	 * {@link ValueValidator Value} Node that is returned from
	 * this method.
	 * 
	 * @return The value that is being switched over.
	 */
	public ValueValidator getControlValue()
	{
		return (ValueValidator)getChild(super.getNumDefaultChildren() + 0);
	}
	...
	public void addChild(NodeValidator nodeValidator)
	{
		if (!isDecoding())
		{
			if (nodeValidator instanceof Default)
			{
				
			}
			else if (nodeValidator instanceof Case)
			{
				if (getLastVisibleChild() instanceof Default)
				{
					SyntaxMessage.error("'default' statement must be the last statement in a switch", nodeValidator);
				}
				
				Case c = (Case) nodeValidator;
			...
			{
				SyntaxMessage.error("Match statements only allow 'case' and 'default' statements as children", nodeValidator);
			}
		}
	...
	public boolean requiresLocalFallthroughVariable()
	{
		...
		{
			NodeValidator child = getScope().getVisibleChild(i);
	 ...
	 * 		into a {@link Match}.
	 */
	public static Match decodeStatement(NodeValidator parent, String statement, Location location, boolean require)
	{
	...
	{
		ValueValidator value = SyntaxTree.decodeValue(getParent(), contents, getLocationIn().asNew(), require);
...
//				Assignment decl = Assignment.decodeStatement(getParent(), , location, require)
			...
			if (getParent() instanceof Assignment) {
				...
				getScope().getVisibleChildren().forEach((child) -> {
					NodeValidator lastChild = child.getScope().getLastVisibleChild();
	...
	private void expressionError(NodeValidator n) {
		SyntaxMessage.error("Invalid case value '" + n.generateFlatInput() + "' for expression", this);
	}

	private ValueValidator getTypeInCommon() {
		java.util.List<NodeValidator> nodeValidators = getScope().getVisibleChildren().stream()
			.map(child -> child.getScope().getLastVisibleChild())
			.filter(child -> child instanceof Throw == false)
			.collect(Collectors.toList());

		if (nodeValidators.get(0) instanceof ValueValidator == false) {
			expressionError(nodeValidators.get(0));
			return null;
		}

		ValueValidator current = null;

		for (NodeValidator n : nodeValidators) {
			if (n instanceof ValueValidator == false) {
				expressionError(n);
				return null;
			}
			ValueValidator value = ((ValueValidator) n).getReturnedNode();
	...
	public void onStackPopped(NodeValidator popped) {
		if (getParent() instanceof Assignment) {
			...
			ValueValidator common = getTypeInCommon();
	 ...
	 */
	@Override
	public Match clone(NodeValidator temporaryParent, Location locationIn, boolean cloneChildren, boolean cloneAnnotations)
	{
