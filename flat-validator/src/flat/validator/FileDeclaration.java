...
public class FileDeclaration extends NodeValidator
{
	...
	public ArrayList<ClosureDeclarationValidator> closures;
	 ...
	 */
	public FileDeclaration(NodeValidator temporaryParent, Location locationIn, File file)
	{
	...
	public ClosureDeclarationValidator[] getPublicClosures()
	{
		ArrayList<ClosureDeclarationValidator> closures = new ArrayList<>();
		
		for (int i =  0; i < this.closures.size(); i++)
		{
			ClosureDeclarationValidator c = this.closures.get(i);
		...
		return closures.toArray(new ClosureDeclarationValidator[0]);
	}
	 ...
	 */
	public int registerClosure(ClosureDeclarationValidator closure)
	{
	...
	public void unregisterClosure(ClosureDeclarationValidator closure)
	{
	...
	public ClassDeclarationValidator getImportedClass(NodeValidator from, String className)
	{
		if (className == null)
		{
			return null;
		...
		ClassDeclarationValidator clazz = from.getParentClass(true);
		...
		for (ClassDeclarationValidator c : from.getFileDeclaration().getClassDeclarations())
		{
	 ...
	 */
	public ClassDeclarationValidator getClassDeclaration(String className)
	{
		for (int i = 0; getNumChildren() > super.getNumDefaultChildren() + getClassOffset() + i; i++)
		{
			ClassDeclarationValidator c = (ClassDeclarationValidator)getChild(super.getNumDefaultChildren() + getClassOffset() + i);
	...
	public ClassDeclarationValidator[] getClassDeclarations()
	{
		ArrayList<ClassDeclarationValidator> classes = new ArrayList<>();
		
		for (int i = 0; getNumChildren() > super.getNumDefaultChildren() + getClassOffset() + i; i++)
		{
			classes.add((ClassDeclarationValidator)getChild(super.getNumDefaultChildren() + getClassOffset() + i));
		}
...
//				SyntaxMessage.error("Missing class declaration", this);
	 ...
	 */
	public ClassDeclarationValidator getClassDeclaration()
	{
		if (getNumChildren() > super.getNumDefaultChildren() + getClassOffset())
		{
			return (ClassDeclarationValidator)getChild(super.getNumDefaultChildren() + getClassOffset());
		}
	...
	public void mergeFiles(FileDeclaration other, int phase)
	{
		...
		ClassDeclarationValidator thisClass = getClassDeclaration();
		ClassDeclarationValidator otherClass = other.getClassDeclaration();
		...
		for (List thisList : fieldLists) {
			for (NodeValidator fieldNodeValidator : thisList.toArray()) {
				FieldDeclaration field = (FieldDeclaration) fieldNodeValidator;
		...
		for (MethodList thisList : methodLists) {
			for (FlatMethodDeclarationValidator method : thisList.getMethods()) {
				if (method.isUserMade()) {
					...
					MethodDeclarationValidator otherMethod = otherClass.getMethod(new GenericCompatible[] { null }, method.getName(), filter, method.getParameterList().getTypes());
...
//			}
	...
	public void addChild(NodeValidator child)
	{
		...
		{
			ClassDeclarationValidator c = (ClassDeclarationValidator) child;

			java.util.List<ClassDeclarationValidator> dupes = getProgram().getVisibleListChildren().stream()
			...
			if (dupes.size() > 0) {
				String classes = dupes.stream().map(ClassDeclarationValidator::getClassLocation).collect(Collectors.joining(", "));
	...
	public static FileDeclaration generateTemporaryFile(NodeValidator parent, Location locationIn)
	{
	 ...
	 */
	@Override
	public FileDeclaration clone(NodeValidator temporaryParent, Location locationIn, boolean cloneChildren, boolean cloneAnnotations)
	{
	...
	public FileDeclaration cloneTo(FileDeclaration node, boolean cloneChildren, boolean cloneAnnotations)
	{
		...
		for (ClosureDeclarationValidator c : closures)
		{
			node.closures.add(c);
		}
