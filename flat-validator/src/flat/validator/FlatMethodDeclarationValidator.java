...
public class FlatMethodDeclarationValidator extends MethodDeclarationValidator implements ScopeAncestor, ClosureCompatible
{
	...
	public FlatMethodDeclarationValidator overridenMethod;
	private ArrayList<FlatMethodDeclarationValidator> overridingMethods, primitiveOverloads;
	public ArrayList<FlatMethodDeclarationValidator> correspondingPrimitiveOverloads;
	
	public FlatMethodDeclarationValidator genericOverload;
	
	private static HashMap<Integer, Scope> scopes = new HashMap<>();
	
	public VirtualMethodDeclarationValidator virtualMethod;
	
	public ObjectReference objectReference;
	
	/**
	 * @see InstanceDeclarationValidator#InstanceDeclarationValidator(NodeValidator, Location)
	 */
	public FlatMethodDeclarationValidator(NodeValidator temporaryParent, Location locationIn)
	{
	...
	public boolean isExtension() {
		return getParentClass() instanceof ExtensionDeclarationValidator;
	}
	...
	public boolean isSuperCallFrom(ClassDeclarationValidator clazz)
	{
	...
	@Override
	public VariableDeclaration searchVariable(NodeValidator parent, Scope scope, String name, boolean checkAncestors)
	{
	...
	private FlatMethodDeclarationValidator getOverriddenMethod(SearchFilter filter)
	{
		if (getVisibility() == PRIVATE && (this instanceof PropertyMethodValidator == false || getParentClass().getField(getName()) == null || !getParentClass().getField(getName()).isPropertyTrue("inheritedFromTrait")))
		{
			return null;
		}
		
		ClassDeclarationValidator extension = getParentClass().getExtendedClassDeclaration();
		
		if (extension != null)
		{
			String name = this instanceof Constructor ? extension.getName() : getName();
			
			FlatMethodDeclarationValidator method = (FlatMethodDeclarationValidator)extension.getMethod(getContext(), name, filter, getParameterList().getTypes());
		...
		{
			FlatMethodDeclarationValidator method = (FlatMethodDeclarationValidator)inter.getMethod(getContext(), getName(), filter, getParameterList().getTypes());
	...
	{
		return overridingMethods.toArray(new FlatMethodDeclarationValidator[0]);
	}
	 ...
	 */
	public boolean containsOverridingMethod(FlatMethodDeclarationValidator overridingMethod)
	{
		for (int i = 0; i < overridingMethods.size(); i++)
		{
			FlatMethodDeclarationValidator methodDeclaration = overridingMethods.get(i);
	 ...
	 * 		this Method.
	 */
	private void addOverridingMethod(FlatMethodDeclarationValidator overridingMethod)
	{
	...
	{
		ArrayList<FlatMethodDeclarationValidator> list = new ArrayList<>();
		...
		for (FlatMethodDeclarationValidator method : list)
		{
	 ...
	 * 		into a Method.
	 */
	public static FlatMethodDeclarationValidator decodeStatement(NodeValidator parent, String statement, Location location, boolean require)
	{
		if (validateMethodDeclaration(statement))
		{
			...
			FlatMethodDeclarationValidator n = new FlatMethodDeclarationValidator(parent, location);
	...
	private static boolean validateMethodDeclaration(String statement)
	{
		...
		return firstParenthIndex >= 0 && !StringUtils.findNextWord(statement).equals(ExternalMethodDeclarationValidator.PREFIX);
	}
...
//		}
	...
	public FlatMethodDeclarationValidator getExistingConvertedPrimitiveMethod(ValueValidator[] args)
	{
		return getExistingConvertedPrimitiveMethod(args, getMethodGenericTypeParameterDeclaration().getTypes());
	}
	
	public FlatMethodDeclarationValidator getExistingConvertedPrimitiveMethod(ValueValidator[] args, ValueValidator[] methodArgs)
	{
		if (!Flat.PRIMITIVE_OVERLOADS) {
			return null;
		}
		for (FlatMethodDeclarationValidator converted : primitiveOverloads)
		{
			...
			if (params.getNumParameters() == args.length)
			{
				...
				else
				{
					...
					{
						ValueValidator param = genParams.getVisibleChild(i);
						ValueValidator arg = methodArgs[i].getReturnedNode();
...
//		}
	...
	public FlatMethodDeclarationValidator checkConvertToClass(ClassDeclarationValidator type)
	{
		return checkConvertToClass(type, getMethodGenericTypeParameterDeclaration().getTypes());
	}
	
	public FlatMethodDeclarationValidator checkConvertToClass(ClassDeclarationValidator type, ValueValidator[] methodTypes)
	{
		if (!Flat.PRIMITIVE_OVERLOADS) {
			return null;
		}
	...
	public FlatMethodDeclarationValidator convertToClass(ClassDeclarationValidator parent, final ValueValidator[] types, final ValueValidator[] methodTypes)
	{
		if (!Flat.PRIMITIVE_OVERLOADS) {
			return null;
		}
		ClassDeclarationValidator referenceClass = getParentClass();

		FlatMethodDeclarationValidator clone = clone(parent, getLocationIn(), false, true);
...
//		if (changed)
		{
			...
			MethodDeclarationValidator[] found = parent.getMethods(new GenericCompatible[] { parent }, clone.getName(), filter, clone.getParameterList().getTypes(), false);
	...
	public FlatMethodDeclarationValidator convertPrimitiveMethod(MethodCall call, ValueValidator returnType, NodeValidator addTo, ValueValidator[] types, ValueValidator[] methodTypes, ArrayList<ValueValidator[]> closureTypes)
	{
		if (!Flat.PRIMITIVE_OVERLOADS) {
			return null;
		}
		FlatMethodDeclarationValidator method = clone(getParent(), getLocationIn(), false, true);
		...
		methodParams.forEachVisibleListChild(param -> {
			if (SyntaxUtils.isPrimitiveType(param.getDefaultType())) {
				param.setDataType(ValueValidator.VALUE);
			}
		});
		...
		for (int i = 0; i < types.length; i++)
		{
			...
			if (param instanceof ClosureDeclarationValidator)
			{
				...
				ClosureDeclarationValidator closure = (ClosureDeclarationValidator)param;
				
				closure.register();
				
				ValueValidator[] closureValues = closureTypes.get(closureIndex++);
				...
				for (int n = 0; n < closureValues.length; n++)
				{
					...
					for (Variable v : original.references)
					{
						if (v instanceof MethodCall)
						{
							...
							ValueValidator val = c.getArgumentList().getVisibleChild(n);
							
							if (val instanceof Variable)
							{
								VariableDeclaration decl = ((Variable)val).declaration;
								
								if (decl instanceof ReferenceParameter || decl instanceof InstanceDeclarationValidator && decl.getParentClass() == getParentClass())
								{
									addTo = moveToClass(method, call);
								}
							}
	...
	private ClassDeclarationValidator moveToClass(FlatMethodDeclarationValidator original, Variable var)
	{
		ClassDeclarationValidator newClass = var.getDeclaringClass();
	...
	public FlatMethodDeclarationValidator getConvertedPrimitiveMethod(MethodCall call)
	{
		...
		ValueValidator[] args = call.getArgumentList().getArgumentsInOrder();
		ValueValidator[] methodArgs = call.getMethodGenericTypeArgumentList().getTypes();
		
		FlatMethodDeclarationValidator existing = getExistingConvertedPrimitiveMethod(args, methodArgs);
		...
		AccessibleValidator ref = call.getReferenceNode();
		ValueValidator[] argTypes = ref.toValue().getGenericTypeArgumentList() != null ? ref.toValue().getGenericTypeArgumentList().getTypes() : null;
		
		FlatParameterList parameters = getParameterList();
		
		ValueValidator[] types = new ValueValidator[args.length];
		ValueValidator[] methodTypes = new ValueValidator[methodArgs.length];
		ArrayList<ValueValidator[]> closureTypes = new ArrayList<>();
		...
		{
			ValueValidator arg = args[i].getReturnedNode();
			ValueValidator param = parameters.getParameter(i);
...
//		}
		...
		{
			ValueValidator arg = methodArgs[i].getReturnedNode();
			ValueValidator param = methodGenParams.getVisibleChild(i);
			
			isPrimitive |= checkType(methodArgs, methodTypes, i, arg, param, closureTypes);
		}
		
		ValueValidator returnType = call;
		NodeValidator addTo = isInstance() ? ref.toValue().getTypeClass() : call.getInferredDeclaration().getParentClass();//call.getReferenceNode().toValue().getTypeClass();//getParent();
...
//			isPrimitive |= ClassDeclaration.replacePrimitiveGenerics(getMethodGenericTypeParameterDeclaration(), methodArgs, call, call, true);
	...
	public boolean checkType(ValueValidator[] argTypes, ValueValidator[] types, int i, ValueValidator arg, ValueValidator param, ArrayList<ValueValidator[]> closureTypes)
	{
		boolean isPrimitive = false;
		...
		if (param instanceof ClosureDeclarationValidator)
		{
			...
			{
				CallableMethodValidator callable = null;
				...
				{
					if (varg.declaration.getOriginalDeclaration() instanceof CallableMethodValidator == false) {
						return false;
					}

					callable = (CallableMethodValidator)varg.declaration.getOriginalDeclaration();
				}
				
				ParameterList aParams = callable.getParameterList();
				ParameterList<ValueValidator> cParams = closure.getParameterList();
				
				ValueValidator[] closureValues = new ValueValidator[Math.min(aParams.getNumParameters(), cParams.getNumParameters())];
				
				for (int n = 0; n < closureValues.length; n++)
				{
					ValueValidator aarg = aParams.getParameter(n);
					ValueValidator aparam = cParams.getParameter(n);
					...
					else
					{
						...
						{
							closureValues[n] = (ValueValidator)aarg.clone(aarg.getParent(), aarg.getLocationIn(), true, true);
	...
	{
		ValueValidator[] types = new ValueValidator[getNumReturnValues()];
		
		types[0] = getTypeClass();
		
		ValueValidator[] ret = getParameterList().getReturnTypes();
	...
	public String generateFlatClosureReference(IdentifierValidator context)
	{
		String reference = "";
	...
	public void searchVirtualMethodDeclaration()
	{
		...
		{
			virtualMethod = new VirtualMethodDeclarationValidator(getParentClass(), getLocationIn().asNew());
...
//			getParentClass().addChild(virtualMethod);
	...
	public void decodeShorthandAction()
	{
		if (shorthandAction != null && getProgram().decodeShorthandActions)
		{
			...
			NodeValidator contents = SyntaxTree.decodeScopeContents(this, action, getLocationIn(), true);
			
			if (contents instanceof ValueValidator)
			{
				...
				contents = inferShorthandActionType(action, (ValueValidator)contents);
			}
			...
			{
				for (FlatMethodDeclarationValidator m : correspondingPrimitiveOverloads)
				{
	...
	public ValueValidator inferShorthandActionType(String action, ValueValidator contents)
	{
		...
		ValueValidator returned = contents.getReturnedNode();
		...
		if (returned instanceof Closure == false && returned.getType() == null)
		{
			...
			{
				FlatMethodDeclarationValidator method = ((MethodCall)returned).getFlatMethod();
	...
	public boolean overrides(FlatMethodDeclarationValidator other)
	{
		FlatMethodDeclarationValidator method = overridenMethod;
	...
	public void updateGenericParameters()
	{
		if (overridenMethod != null)
		{
			...
			{
				NodeValidator n = getScope().getVisibleChild(0);
				...
				else if (n instanceof AssignmentValidator)
				{
					AssignmentValidator assign = (AssignmentValidator)n;
	...
	public MethodCall[] getReferencesIncludingOverrides()
	{
		...
		for (FlatMethodDeclarationValidator method : getOverridingMethods())
		{
	...
	public ValidationResult validate(int phase)
	{
		...
		else if (phase == SyntaxTree.PHASE_PRE_GENERATION)
		{
			...
			if (this instanceof VirtualMethodDeclarationValidator == false && virtualMethod != null)
			{
	...
	private void checkOverloads()
	{
		if (overloadID < 0)
		{
			...
			MethodDeclarationValidator methods[] = getParentClass().getMethods(getName(), filter);
	 ...
	 */
	@Override
	public FlatMethodDeclarationValidator clone(NodeValidator temporaryParent, Location locationIn, boolean cloneChildren, boolean cloneAnnotations)
	{
		FlatMethodDeclarationValidator node = new FlatMethodDeclarationValidator(temporaryParent, locationIn);
	 ...
	 */
	public FlatMethodDeclarationValidator cloneTo(FlatMethodDeclarationValidator node)
	{
		return cloneTo(node, true, true);
	}
	
	/**
	 * Fill the given {@link FlatMethodDeclarationValidator} with the data that is in the
	 * specified node.
	 * 
	 * @param node The node to copy the data into.
	 * @return The cloned node.
	 */
	public FlatMethodDeclarationValidator cloneTo(FlatMethodDeclarationValidator node, boolean cloneChildren, boolean cloneAnnotations)
	{
	...
	{
		for (FlatMethodDeclarationValidator overriding : overridingMethods)
		{
			overriding.overridenMethod = (FlatMethodDeclarationValidator)replacement;
		}
