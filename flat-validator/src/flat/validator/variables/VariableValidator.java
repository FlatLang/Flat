...
import flat.validator.AbstractValueValidator;
import flat.validator.IdentifierValidator;
import flat.validator.NodeValidator;
import flat.validator.ValueValidator;

import java.util.AbstractMap;

public class VariableValidator extends IdentifierValidator
{
	public VariableDeclarationValidator declaration;
	 ...
	 */
	public VariableValidator(NodeValidator temporaryParent, Location locationIn)
	{
	...
	{
		AbstractMap.SimpleEntry<ValueValidator, Boolean> cast = getIfStatementCast();
	...
	public AbstractMap.SimpleEntry<ValueValidator, Boolean> getBinaryStatementCast()
	{
	...
	public AbstractMap.SimpleEntry<ValueValidator, Boolean> getIfStatementCast()
	{
		...
		if (current != null)
		{
			...
			if (current != null)
			{
				...
				while (current != null)
				{
					NodeValidator[] children = current.getCondition().getChildrenOfType(MethodCall.class);
					
					for (NodeValidator n : children)
					{
						...
						if (m.getName().equals("isOfType") && m.getDeclaringClass().getClassLocation().endsWith("flat/meta/Class"))
						{
							...
							if (r instanceof VariableValidator && ((VariableValidator)r).declaration == declaration)
							{
								Accessible root = m.getRootAccessNode();
								NodeValidator parent = root.getParent();
								
								boolean negated = parent instanceof UnaryOperation && ((UnaryOperation)parent).getOperator().getOperator().equals(Operator.BANG);
								
								ValueValidator cloneType = ((VariableValidator)r).cloneTo(new IValue(parent, getLocationIn()), true, true, true);
...
//					}
	...
	public void onReplaced(NodeValidator parent, NodeValidator replacement)
	{
		if (parent.getAncestorOfType(Assignment.class, true) != null)
		{
			...
			VariableDeclarationValidator decl = a.getAssignedNode().getDeclaration();
			...
			if (decl instanceof LocalDeclaration)
			{
				...
				{
					local.setImplicitType((ValueValidator)replacement);
				}
			}
		}
	 ...
	 * 		declaration.
	 */
	public boolean isSameVariable(VariableValidator other)
	{
		VariableDeclarationValidator first  = getDeclaration();
		VariableDeclarationValidator second = other.getDeclaration();
...
//	}
	 ...
	 * 		specified variable.
	 */
	public VariableDeclarationValidator getDeclaration()
	{
		return declaration;
	}
	 ...
	 * 		specified variable.
	 */
	public void setDeclaration(VariableDeclarationValidator declaration)
	{
	...
	/**
	 * @see IdentifierValidator#setName(String, boolean)
	 */
	@Override
	...
	/**
	 * @see IdentifierValidator#doesForceOriginalName()
	 */
	@Override
	...
	/**
	 * @see IdentifierValidator#setForceOriginalName(boolean)
	 */
	@Override
	...
	/**
	 * @see ValueValidator#getArrayDimensions()
	 */
	@Override
	...
	/**
	 * @see ValueValidator#setArrayDimensions(int)
	 */
	@Override
	...
	public void swapNames(VariableValidator other)
	{
		swapNames(other.getDeclaration());
	}
	
	public void swapNames(VariableDeclarationValidator other)
	{
		getDeclaration().swapNames(other);
	}
	
	/**
	 * @see ValueValidator#getType()
	 */
	@Override
	...
	/**
	 * @see AbstractValueValidator#setTypeValue(String)
	 */
	@Override
	...
	/**
	 * @see ValueValidator#setType(String, boolean, boolean, boolean)
	 */
	@Override
	...
	public byte getDataType(boolean checkGeneric, boolean checkCast)
	{
		...
		{
			ValueValidator value = getFlatTypeValue(this);
	...
	/**
	 * @see ValueValidator#setDataType(byte)
	 */
	@Override
	...
	/**
	 * @see VariableDeclarationValidator#isVolatile()
	 */
	...
	/**
	 * @see VariableDeclarationValidator#setVolatile(boolean)
	 */
	...
	@Override
	public GenericTypeArgument getGenericTypeArgument(int index, NodeValidator value, boolean require)
	{
...
//		}
	...
	public VariableDeclarationValidator getReferenceDeclaration()
	{
		Accessible c = getCorrespondingImplicitValue();
		
		if (c != null && c instanceof VariableValidator)
		{
			return ((VariableValidator)c).getDeclaration();
		}
	...
	public Accessible getCorrespondingImplicitValue()
	{
		...
		{
			ValueValidator v = ((LocalDeclaration)declaration).correspondingImplicit;
	...
	public ValidationResult validate(int phase)
	{
		...
		if (phase >= SyntaxTree.PHASE_METHOD_CONTENTS)
		{
			if (getDeclaration() != null)
			{
				...
				{
					IdentifierValidator accessed = getAccessedNode();
					
					if (accessed.getName().equals("class"))
					{
						IdentifierValidator nextAccessed = accessed.getAccessedNode();
						...
						if (nextAccessed instanceof MethodCall && nextAccessed.getName().equals("isOfType"))
						{
							...
							ValueValidator[] args = argList.getArgumentsInOrder();
			...
			if (getDeclaration() instanceof FieldDeclarationValidator)
			{
				FieldDeclarationValidator field    = (FieldDeclarationValidator)getDeclaration();
	...
	private ValueValidator replaceWithPrimitiveStaticCall()
	{
		...
		ValueValidator staticCall = SyntaxTree.decodeIdentifierAccess(root.getParent(), syntax, call.getLocationIn(), true, false).toValue();
		
		IdentifierValidator accessed = call.getAccessedNode();
	 ...
	 */
	@Override
	public VariableValidator clone(NodeValidator temporaryParent, Location locationIn, boolean cloneChildren, boolean cloneAnnotations)
	{
		VariableValidator node = new VariableValidator(temporaryParent, locationIn);
	 ...
	 */
	public VariableValidator cloneTo(VariableValidator node)
	{
		return cloneTo(node, true, true);
	}
	
	/**
	 * Fill the given {@link VariableValidator} with the data that is in the
	 * specified node.
	 * 
	 * @param node The node to copy the data into.
	 * @return The cloned node.
	 */
	public VariableValidator cloneTo(VariableValidator node, boolean cloneChildren, boolean cloneAnnotations)
	{
