...
public class ExtensionFieldDeclaration extends FieldDeclaration {
    ...
    /**
     * Decode the given statement into a {@link ExtensionFieldDeclaration} instance, if possible. If
     ...
     * @param statement The statement to try to decode into a {@link ExtensionFieldDeclaration}
     *        instance.
     * @param location The location of the statement in the source code.
     * @param require Whether or not to throw an error if anything goes wrong.
     * @return The generated node, if it was possible to translated it into a
     *         {@link ExtensionFieldDeclaration}.
     */
    public static ExtensionFieldDeclaration decodeStatement(Node parent, String statement,
        ...
        if (parent.getParentClass(true) instanceof ExtensionDeclaration) {
            ...
            if (bounds.getStart() > 0) {
                ...
                if (dotIndex < eqIndex || eqIndex < 0) {
                    ...
                    statement = statement.substring(0, bounds.getStart()).trim() + " "
                        + statement.substring(dotIndex + 1).trim();

                    FieldDeclaration field =
                        FieldDeclaration.decodeStatement(parent, statement, location, require);

                    if (field != null) {
                        ExtensionFieldDeclaration n =
                            new ExtensionFieldDeclaration(parent, location);

                        field.cloneTo(n, true, true);

                        /*
                         * Parameter type =
                         * (Parameter)n.getParameterList().getParameter(0).detach();
                         * 
     ...
     */
    @Override
    public ExtensionFieldDeclaration clone(Node temporaryParent, Location locationIn,
    ...
    /**
     * Fill the given {@link ExtensionFieldDeclaration} with the data that is in the specified node.
     *
    ...
    /**
     * Test the {@link ExtensionFieldDeclaration} class type to make sure everything is working
     * properly.
     *
