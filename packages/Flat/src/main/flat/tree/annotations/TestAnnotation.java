...
public class TestAnnotation extends Annotation
    ...
    public static TestAnnotation decodeStatement(Node parent, String name, String parameters,
        ...
        if (phase == SyntaxTree.PHASE_METHOD_CONTENTS) {
            if (parameters.containsKey("message")) {
                ...
                if (!message.isNullLiteral() && !message.value.equals("false")
                    ...
                    if (c != ' ' && c != '\t' && c != '\n' && c != '\r') {
                        message.value =
                            message.value.substring(0, message.value.length() - 1) + " \"";
                    }
                ...
                String name =
                    method.getLiteralNameData() != null ? method.getLiteralNameData().literalName
                        : method.getName();

                writeMessage((Literal) Literal.decodeStatement(parent, "\"Testing " + name + " \"",
    ...
    public FieldDeclaration generateTestCase() {
        if (testCase == null) {
            ...
            testCase = addFieldInitialization("TestCase", name,
                "TestCase(\"" + className + "\", \"" + testName + "\"" + description + ")");
    ...
    @Override
    public TestAnnotation clone(Node temporaryParent, Location locationIn, boolean cloneChildren,
    ...
    public TestAnnotation cloneTo(TestAnnotation node, boolean cloneChildren,
        boolean cloneAnnotations) {
