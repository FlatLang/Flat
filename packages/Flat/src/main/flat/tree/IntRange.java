...
public class IntRange extends Instantiation {
    ...
    /**
     * Decode the given statement into a {@link IntRange} instance, if possible. If it is not
     ...
     * @param statement The statement to try to decode into a {@link IntRange} instance.
     * @param location The location of the statement in the source code.
     * @param require Whether or not to throw an error if anything goes wrong.
     * @return The generated node, if it was possible to translated it into a {@link IntRange}.
     */
    public static IntRange decodeStatement(Node parent, String statement, Location location,
        boolean require) {
        ...
        if (operatorIndex > 0) {
             ...
             * 
             * if (startValue == null) { SyntaxMessage.queryError("Invalid start value '" +
             * startValueString + "' for integer range", n, require);
             * 
             * return null; }
             * 
             * Value endValue = SyntaxTree.decodeValue(n, endValueString, location, require);
             * 
             * if (endValue == null) { SyntaxMessage.queryError("Invalid end value '" +
             * endValueString + "' for integer range", n, require);
             * 
             * return null; }
             */
            ...
            Instantiation inst = Instantiation.decodeStatement(parent,
                "IntRange(" + startValueString + ", " + endValueString + ")", location, false);

            if (inst == null) {
                // SyntaxMessage.queryError("Unable to decode integer range '" + statement + "'", n,
                // require);
    ...
    /**
     * Fill the given {@link IntRange} with the data that is in the specified node.
     *
    ...
    /**
     * Test the {@link IntRange} class type to make sure everything is working properly.
     *
