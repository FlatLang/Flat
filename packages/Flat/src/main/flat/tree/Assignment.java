...
/**
 * Node extension that contains information describing a variable assignment.
 *
...
public class Assignment extends Value {
    ...
    /**
     * Get the node that is having its value modified. In other words, the left hand value of the
     * equation. For instance, in the statement: "<code>Int j = 35</code>" <u><code>Int j</code></u>
     * is the left hand value of the equation.
     *
     * @return The node that represents the variable that is being assigned.
     */
    ...
    public Value getFlatTypeValue(Value context) {
        return getAssignmentNode().getReturnedNode()
            .getFlatTypeValue(getAssignmentNode().getReturnedNode());
    }
    ...
    /**
     * Get the node that is being used to set the value of the assignee node. In other words, the
     * right hand value of the equation. For instance, in the statement: "<code>int j = 35</code>"
     * <u><code>35</code></u> is the right hand value of the equation.
     *
     * @return The node that represents the value that the assignee variable is being assigned to.
     */
    ...
    public StringBuilder generateFlatInput(StringBuilder builder, boolean outputChildren) {
        return getAssigneeNode().generateFlatInput(builder, outputChildren).append(" = ")
            .append(getAssignmentNode().generateFlatInput(outputChildren));
    }
    ...
    /**
     * Decode the given statement into an Assignment if possible. If it is not possible, then null
     * is returned.<br>
     * <br>
     ...
     * @return The new Assignment if it decodes properly. If not, it returns null.
     */
    public static Assignment decodeStatement(Node parent, String statement, Location location,
        boolean require) {
     ...
     * @param location The location of the statement in the source code.
     * @param require Whether or not to throw an error if anything goes wrong.
     * @param addDeclaration Whether or not to add the declaration to the nearest scope, if the left
     *        hand value of the equation is a variable declaration.
     * @return The new Assignment if it decodes properly. If not, it returns null.
     */
    public static Assignment decodeStatement(Node parent, String statement, Location location,
        boolean require, boolean addDeclaration) {
     ...
     */
    public static Assignment decodeStatement(Node parent, String statement, Location location,
        boolean require, boolean addDeclaration, Value[] assignees, Node assignment) {
        return decodeStatement(parent, statement, location, require, addDeclaration, assignees,
            assignment, true);
    }
    ...
    public static Assignment decodeStatement(Node parent, String statement, Location location,
        ...
        if (rhsIndex > 0) {
            ...
            newLoc.setBounds(location.getStart() + rhsIndex,
    ...
    public boolean onAfterDecoded() {
        if (super.onAfterDecoded()) {
            ...
            if (getAssignedNodeValue().getType() != null && getAssignedNodeValue().isImmutable()
                && getAssignedNodeValue() instanceof Variable) {
                if (getAssignedNode().getImmutableAnnotation() != null) {
                    getAssignedNode().getImmutableAnnotation()
                        .convertAssignment(getAssignmentNode());
                }
            }
    ...
    public void onAdded(Node parent) {
        if (wasDeclaration) {
            if (parent instanceof Scope == false
                && !(parent instanceof ArgumentList && parent.parent instanceof ForLoop) ||
                getAssignmentNode() instanceof ControlStatement) {
                getAssignedNode().getDeclaration().setProperty("requiresPrecedingDeclaration",
                    true);
            }
        }
        if (getAssignedNodeValue() instanceof Variable
            && getAssignedNode().getDeclaration() instanceof ArrayAccessorMethod) {
    ...
    private boolean decodeAssignee(String assignee, Location loc, boolean require,
        boolean addDeclaration, Value[] assignees, boolean checkType) {
        if (assignees == null || assignees.length <= 0) {
            ...
            if (varNode == null) {
                ...
                if (varNode == null) {
                    return SyntaxMessage.queryError("Undeclared variable '" + assignee + "'", this,
                        loc, require);
                }
            }
            ...
            if (declaration.isImplicit()) {
                ...
                if (declaration.getImplicitType() == null) {
                    ...
                    return SyntaxMessage.queryError(
                        "Implicit type for '" + declaration.getName() + "' cannot be determined",
                        this, require);
                    ...
                    return SyntaxMessage.queryError(
                        "Incompatible array assignment. Assigned node has " + declarationDimensions
                            + " dimension" + (declarationDimensions == 1 ? "" : "s")
                            + ", when assignment has " + assignmentDimensions + " dimension"
                            + (assignmentDimensions == 1 ? "" : "s"),
                        this, require);
                } else if (previousType.getTypeClass()
                    .hasCommonAncestor(assignment.getTypeClass())) {
                    ClassDeclaration base =
                        previousType.getTypeClass().getCommonAncestor(assignment.getTypeClass());
                    ...
                    getAncestorWithScope().getScope().addImplicitVariableAssignment(declaration,
                        base, assignment);
                } else {
                    previousType.getTypeClass();
                    assignment.getTypeClass();
                    return SyntaxMessage.queryError(
                        "Incompatible implicit type '" + declaration.getType()
                            + "' being assigned to type '" + assignment.getType() + "'",
                        this, require);
                }
                ...
                if (assignment.getType() != null
                    && (assignment.isPrimitive() && assignment.getType().equals("Byte")
                        || assignment.getType().equals("Short"))) {
                ...
                // {
                // declaration.getGenericTypeArgumentList().addChild(arg.clone(null,
                // declaration.getLocationIn()));
                // }
                // }
                // else
                // {
        ...
        if (!Literal.isNullLiteral(returnedRight)) {
            if (rightType != null && !rightType.equals(leftType)) {
                ...
                if (!returnedLeft.isExternalType() && !returnedRight.isExternalType() &&
                    ...
                    SyntaxMessage.error("Type '" + returnedRight.getType()
                        + "' is not compatible with type '" + returnedLeft.getType() + "'", this);
                }
            }

            /*
             * if (!SyntaxUtils.isTypeCompatible(returnedRight.getContext(), returnedLeft,
             * returnedRight, true)) { SyntaxUtils.isTypeCompatible(returnedRight.getContext(),
             * returnedLeft, returnedRight, true); }
             * 
             * return SyntaxUtils.isTypeCompatible(returnedRight.getContext(), returnedLeft,
             * returnedRight, true);
             */
        }
        ...
        if (phase == SyntaxTree.PHASE_METHOD_CONTENTS) {
            if (containsMultipleAssignees()) {
                ...
                if (!(assignment instanceof MethodCall)) {
                    SyntaxMessage.error(
                        "Multiple assignments to single value is not implemented yet", this, false);
                ...
                if (!SyntaxUtils.areTypesCompatible(contexts.toArray(new GenericCompatible[0]),
                    assignment.getTypes(), getAssigneeNodes().getTypes())) {
                    SyntaxMessage.error("Invalid types to be assigned from method call", this,
                        false);
                    ...
                    if (mutator != null && getParentMethod() != mutator) {
                        if (!mutator.isDisabled()) {
                            String text = field.getName() + "("
                                + getAssignmentNode().generateFlatInput() + ")";

                            MethodCall call = MethodCall.decodeStatement(getParent(), text,
                                getLocationIn(), true, false, mutator);

                            call.getArgumentList().getVisibleChild(0)
                                .replaceWith(getAssignmentNode());
                            ...
                            SyntaxMessage.error(
                                "The field '" + field.getName() + "' is not settable", this, false);
    ...
    /**
     * Validate that the assignment is authorized to assign a value to the given variable in the
     * assignments location. The variable is not authorized to be modified under the following
     * condition:<br>
     * <u>The variable cannot be modified if it is <b>private</b> or <b>visible</b> and is not
     * contained within the same class as the assignment.</u>
     *
        ...
        if (id instanceof Variable) {
            ...
            if (id.isAccessed() && declaration instanceof FieldDeclaration) {
                ...
                if (field.getVisibility() == FieldDeclaration.VISIBLE) {
                    ...
                    {
                        SyntaxMessage.error(
                            "The value of the field '" + field.getName() + "' cannot be modified",
                            id);
                    }
                }
            }
    ...
    /**
     * Add the variable declaration to the parent scope if the assignment was also a
     * declaration.<br>
     * <br>
     ...
     * </pre>
     * 
     * </blockquote> Scenario 1 includes a declaration and therefore, this method would add that
     * declaration to its parent scope and return a clone of the declared Variable. On the other
     * hand, scenario 2 does not include a declaration and therefore simply does nothing in this
     * method (Returns the given Variable).
     *
     * @param var The Variable to check whether or not declares a variable.
     * @return If a variable is declared, this returns a clone of the declared variable. If not,
     *         this simply returns the given Variable instance.
     */
    ...
    /**
     * Decode the right hand side of an assignment into an Node if possible. If it is not possible,
     * then null is returned. The right hand side of an assignment must represent a value.<br>
     * <br>
     * Example inputs include:<br>
     * <ul>
     * <li>45</li>
     ...
     * @param location The location of the statement in the source code.
     * @param require Whether or not to throw an error if anything goes wrong.
     * @return The new Node if it decodes properly. If not, it returns null.
     */
    public Value decodeRightHandSide(Node parent, String rhs, Location location, boolean require) {
        ...
        if (!(child instanceof Value)) {
            SyntaxMessage.queryError("Could not decode assignment right-hand side '" + rhs + "'",
                parent, location, require);
    ...
    private Value checkPrimitive(Value value) {
        if (getAssignedNodeValue().getReturnedNode().getType() != null) {
            if (getAssignedNodeValue().getReturnedNode().isPrimitive()) {
                if (!value.getReturnedNode().isPrimitive() && getParentMethod() != null
                    && getParentMethod().getProperty("array") == null) {
                    return SyntaxUtils.unboxPrimitive(value,
                        value.getReturnedNode().getFlatType(value, false));
                }
    ...
    /**
     * Fill the given {@link Assignment} with the data that is in the specified node.
     *
    ...
    /**
     * Test the Assignment class type to make sure everything is working properly.
     *
