...
/**
 * {@link FlatMethodDeclaration} extension that represents the declaration of an abstract method
 * node type. See {@link #decodeStatement(Node, String, Location, boolean)} for more details on what
 * correct inputs look like.
 *
...
public class AbstractMethodDeclaration extends FlatMethodDeclaration {
    ...
    /**
     * Decode the given statement into a {@link AbstractMethodDeclaration} instance, if possible. If
     ...
     * @param statement The statement to try to decode into a {@link AbstractMethodDeclaration}
     *        instance.
     * @param location The location of the statement in the source code.
     * @param require Whether or not to throw an error if anything goes wrong.
     * @return The generated node, if it was possible to translated it into a Method.
     */
    public static AbstractMethodDeclaration decodeStatement(Node parent, String statement,
        Location location, boolean require) {
        if (SyntaxUtils.findStringInBaseScope(statement, IDENTIFIER) >= 0) {
            statement =
                findMethodSignature(statement, StringUtils.findWordBounds(statement, IDENTIFIER));
        } else if (!parent.getFileDeclaration()
            .containsPendingAnnotationOfType(AbstractAnnotation.class)) {
            return null;
        }
        ...
        if (statement != null && statement.length() > 0) {
            ...
            FlatMethodDeclaration method =
                FlatMethodDeclaration.decodeStatement(n, statement, location.asNew(), require);
    ...
    public ValidationResult validate(int phase) {
        ...
        if (phase == SyntaxTree.PHASE_INSTANCE_DECLARATIONS) {
            if (!getParentClass().isAbstract()) {
                ...
                SyntaxMessage
                    .error(
                        "The class '" + getParentClass().getName() + "' must be " + IDENTIFIER
                            + " to contain the " + IDENTIFIER + " method " + getName(),
                        this, false);
            }
    ...
    /**
     * Fill the given {@link AbstractMethodDeclaration} with the data that is in the specified node.
     *
    ...
    /**
     * Test the {@link AbstractMethodDeclaration} class type to make sure everything is working
     * properly.
     *
